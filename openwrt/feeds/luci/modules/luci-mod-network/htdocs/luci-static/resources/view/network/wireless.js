...
function format_wifirate(rate) {
	var s = '%.1f\xa0%s, %d\xa0%s'.format(rate.rate / 1000, _('Mbit/s'), rate.mhz, _('MHz')),
	    ht = rate.ht, vht = rate.vht, he = rate.he,
	...
	if (ht || vht || he) {
		...
		if (he)  s += ', HE-MCS\xa0%d'.format(mcs);
	}
...
var CBIWifiFrequencyValue = form.Value.extend({
	...
	load: function(section_id) {
		return Promise.all([
			...
			if (htmodelist.HE160)
				if (this.channels['11a'].length < 9 * 3)
					delete htmodelist.HE160;
			if (htmodelist.VHT160)
				if (this.channels['11a'].length < 9 * 3)
					delete htmodelist.VHT160;

			if (htmodelist.HE80)
				if (this.channels['11a'].length < 5 * 3)
					delete htmodelist.HE80;
			if (htmodelist.VHT80)
				if (this.channels['11a'].length < 5 * 3)
					delete htmodelist.VHT80;

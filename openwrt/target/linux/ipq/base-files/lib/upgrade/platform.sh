...
do_flash_firmware(){
    ...
    local os_idx=$(/usr/sbin/nvram get flag_boot_rootfs)

    fwtool -i /tmp/sysprade.meta -t $1
    fwtool -s /tmp/sysprade.ucert -t $1
    if [ "$os_idx" == "0" ];then
        mtdname=rootfs_1
        mtdpart=$(grep "\"${mtdname}\"" /proc/mtd | awk -F: '{print $1}')
        ubiformat /dev/${mtdpart} -y -f $1
    else
         mtdname=rootfs
         mtdpart=$(grep "\"${mtdname}\"" /proc/mtd | awk -F: '{print $1}')
         ubiformat /dev/${mtdpart} -y -f $1
...
platform_check_image() {
    fwtool -i /tmp/sysupgrade.metadata "$1"
    local flag1=`echo "$?"`
    fwtool -s /tmp/sysupgrade.ucert "$1"
    local flag2=`echo "$?"`

    if [ "$flag1" = "0" -a "$flag2" = "0" ];then
        return 0
    else
        echo "Error: \"$1\" format error. Abort..."
        return 1
    fi
	local board=$(ipq806x_board_name)
...
platform_do_upgrade() {
	...
	case "$board" in
	db149 | ap148 | ap145 | ap148_1xx | db149_1xx | db149_2xx | ap145_1xx | ap160 | ap160_2xx | ap161 | ak01_1xx | ap-dk01.1-c1 | ap-dk01.1-c2 | ap-dk04.1-c1 | ap-dk04.1-c2 | ap-dk04.1-c3 | ap-dk04.1-c4 | ap-dk04.1-c5 | ap-dk04.1-c6 | ap-dk05.1-c1 |  ap-dk06.1-c1 | ap-dk07.1-c1 | ap-dk07.1-c2 | ap-dk07.1-c3 | ap-dk07.1-c4 | ap-hk01-c1 | ap-hk01-c2 | ap-hk01-c3 | ap-hk01-c4 | ap-hk01-c5 | ap-hk01-c6 | ap-hk02 | ap-hk06 | ap-hk07 | ap-hk08 | ap-hk09 | ap-hk10-c1 | ap-hk10-c2 | ap-hk11-c1 | ap-hk12 | ap-ac01 | ap-ac02 | ap-ac03 | ap-ac04 | ap-oak02 | ap-oak03 | db-hk01 | db-hk02 | ap-cp01-c1 | ap-cp01-c2 | ap-cp01-c3 | ap-cp01-c4 | ap-cp02-c1 | ap-cp03-c1 | db-cp01 | db-cp02 | mp-emu | ap-mp02.1 | ap-mp03.1 | ap-mp03.1-c2 | ap-mp03.1-c3 | ap-mp03.3 | ap-mp03.3-c2 | ap-mp03.6-c1 | ap-mp03.6-c2 | db-mp02.1 | db-mp03.1 | db-mp03.1-c2 | db-mp03.3 | db-mp03.3-c2)

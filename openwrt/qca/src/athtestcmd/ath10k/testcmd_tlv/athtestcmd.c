...
void DispHexString(unsigned char *pkt_buffer, unsigned int recvsize)
{
  int i, j, k;
...
void rateIndexToArrayMapping (A_UINT32 rateIndex, A_UINT32 *rowIndex, A_UINT32 *bitmask, A_BOOL *is11AcRate )
{
    *is11AcRate = FALSE;
    ...
    printf("Is11AcRate %d rateIndex=%d\n",*is11AcRate, rateIndex);
...
int main (int argc, char **argv)
{
    ...
    while (1) {
        ...
        c = getopt_long(argc, argv, "vi:t:f:g:h:H:I:j:z:e:c:C:b:d:D:r:p:q:k:K:x:s:X:aAYZouU:S:Gn:N:M:m:l:P:",
...
//#define M_EEEPROM_BLOCK_WRITE_ID 201
	     ...
	     printf("TCMD_RX_RATE\n");
            {
                ...
                A_UINT8 rateMask[10];
                value = atoi(optarg);
                if((value >= ATH_RATE_1M ) &&  (value <= MASK_RATE_MAX )) {
    ...
    DispHexString(buf, cmdStreamLen);

...
extern A_UINT32 g_eeprom_size;
extern A_INT8 g_eeprom_read_done;
extern A_UINT8 bdata[BOARD_DATA_SZ_MAX];
extern A_UINT8 responseStatus;
...
#ifdef USE_NETLINK
void cmdReplyFunc_v2(void *buf, int len)
#else
...
#ifdef USE_NETLINK
    length = len;
    cmdStreamParsingAndProcessing((A_UINT8*)&reply[0], length, &pPayload, &payloadLen);
#else
...
void handleMEMREADRSP(void *parms)
{
    ...
    if (pParms->status == 0)
    {
        ...
        for (i = 0; i < pParms->numbytes; i++)
        {
            ...
            {
                vptr = (void *)(&pParms->memvalue[i]);
                A_UINT16 *val = (A_UINT16 *)vptr;
                printf("mem[%x]=0x%04x", pParms->memaddress + i, *val);
            ...
            {
                vptr = (void *)(&pParms->memvalue[i]);
                A_UINT32 *val = (A_UINT32 *)vptr;
                printf("mem[%x]=0x%08x", pParms->memaddress + i, *val);

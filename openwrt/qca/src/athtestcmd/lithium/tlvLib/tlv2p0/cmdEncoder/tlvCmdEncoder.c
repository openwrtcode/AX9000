...
TLV_ENCODER_ENUM tlv2CreateCmdHeaderInternal(A_UINT32 maxSegSize, A_UINT32 maxStreamSize, A_UINT32 cmdCode)
{
    ...
    if (streamPos != 0)
    {
        A_PRINTF_ALWAYS("A TLV2 command contruction is already in progress streampos %d\n",streamPos);
    ...
    if (numSegments > 1)
    {
        CmdStreamV2.cmdStreamHeader.header |= CMD_STREAM_HEADER_EXTENDED_SEGMENT;
            ...
            //segment number: the last byte of extended header
                        CmdStreamV2.cmdStreamHeader.headerExtended = ((currentSegment << CMD_STREAM_HEADER_EXTENDED_SEGMENT_NUM_SHIFT_255) & CMD_STREAM_HEADER_EXTENDED_SEGMENT_NUM_MASK_255) |
                                ((numSegments << CMD_STREAM_HEADER_EXTENDED_TOTAL_SEGMENTS_SHIFT_255) & CMD_STREAM_HEADER_EXTENDED_TOTAL_SEGMENTS_MASK_255) |

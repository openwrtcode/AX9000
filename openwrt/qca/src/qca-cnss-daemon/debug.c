...
void daemon_qmihist_record(uint8_t instance_id, char msg_id,
			   signed char err_msg, signed char resp_err_msg)
{
	char qmi_record[4];
	if (daemon_debuglog_file >= 0){
		pthread_mutex_lock(&(qmi_record_mutex));
		qmi_record[0] = instance_id;
		qmi_record[1] = msg_id;
		if (err_msg < 0 || resp_err_msg != 0)
			qmi_record[2] = err_msg;
		else
			qmi_record[2] = 0;
		qmi_record[3] = resp_err_msg;

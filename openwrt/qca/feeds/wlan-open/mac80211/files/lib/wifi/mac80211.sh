...
EOF
	...
	for _dev in /sys/class/ieee80211/*; do
		...
		local l_disable
		if [ $devidx -eq 0 ]; then
			l_ssid="'AX9000_OpenWrt'"
			l_disable="'1'"
			ht_capab="	option htmode 'HT20'"
		elif [ $devidx -eq 1 ]; then
			l_ssid="'AX9000_OpenWrt-5G_Game'"
			l_disable="'0'"
			ht_capab="	option htmode 'HE80'"
		elif [ $devidx -eq 2 ]; then
			l_ssid="'AX9000_OpenWrt-5G'"
			l_disable="'0'"
			ht_capab="	option htmode 'HE80'"
		elif [ $devidx -eq 3 ]; then
			l_ssid="'AX9000_OpenWrt-2.4G'"
			l_disable="'0'"
			ht_capab="	option htmode 'HE20'"
		fi
